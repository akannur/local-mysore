...
;( function( $, window, undefined ) {
	...
	*/
	...
	$.Stapel.prototype = {
			...
			for( var pile in this.piles ) {
				...
				for( var i = 0, len = p.elements.length; i < len; ++i ) {
					...
					if( i === len - 2 ) {
						...
						$el.data( 'front', true ).append( '<div class="tp-title"><span>' + pile + '</span><span>' + (len - 1) + '</span></div>' );
					}
		...
		_openPile : function() {
			// Hack: Hide Default Image
			$(document).find(".displayImage").parent('li').hide();
		...
		_closePile : function() {
			// Hack: Show Default Image
			$(document).find(".displayImage").parent('li').show();
